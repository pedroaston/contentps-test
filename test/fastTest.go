...
func TestFastDelivery(ctx context.Context, ri *DHTRunInfo) error {
	...
	runenv.R().RecordPoint("Avg latency of events - FastDelivery", float64(latFast))
	...
	runenv.R().RecordPoint("Memory used - FastDelivery", float64(finalMem.Used)-float64(initMem.Used))
