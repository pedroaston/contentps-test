...
func NormalScoutTest(runenv *runtime.RunEnv) error {
...
func TestNormalScout(ctx context.Context, ri *DHTRunInfo) error {
	...
	runenv.R().RecordPoint("Number of peers - ScoutSubs normal", float64(len(ri.Node.dht.RoutingTable().GetPeerInfos())))
	runenv.RecordMessage("GroupID >> " + ri.RunInfo.RunEnv.RunParams.TestGroupID)
	runenv.R().RecordPoint("Events received - ScoutSubs normal", float64(nEScout))
	runenv.R().RecordPoint("Avg event latency - ScoutSubs normal", float64(latScout))
	runenv.R().RecordPoint("Avg time to sub - ScoutSubs normal", float64(ps.ReturnSubStats()))
	runenv.R().RecordPoint("CPU used - ScoutSubs normal", finalCpu[0].User-initCpu[0].User)
	runenv.R().RecordPoint("Memory used - ScoutSubs normal", float64(finalMem.Used)-float64(initMem.Used))
